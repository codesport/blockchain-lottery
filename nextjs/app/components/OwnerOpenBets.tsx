...
const OwnerOpenBets = () => {
  ...
  const handleOpenBets = async (event: any) => {
    ...
    console.log(
      "Output from Form Submit " +
        event.target.fromGameDuration.value +
        " minutes"
    );
  ...
  if (theTime) {
    displayTime = new Date(theTime).toLocaleString();
  }
    ...
    <React.Fragment>
      ...
      <form id="ownerGameDurationForm" onSubmit={handleOpenBets}>
        <label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          Chain data was last pulled at {displayTime}. Enter Lottery Duration in
